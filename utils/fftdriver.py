...
def fast_pfb(timestream, nchan=2048, ntap=4, window=sinc_hanning):
    ...
    if nspec==int(nspec): nspec=int(nspec)
    else: raise Exception("nspec is {}, should be integer".format(nspec))
    output = np.empty((nspec,lblock//2+1),dtype="complex128", order="c")
    mylib.pfb(timestream.ctypes.data, output.ctypes.data, w.ctypes.data, nspec, nchan, ntap)
